...
export interface OpCensusRow {
  sno: number;
  speciality: string;
  count: number;
}

export interface OpCensusExportOpts {
  rows: OpCensusRow[];
  startDate: Date;
  endDate: Date;
  outputPath: string;
...
export async function generateOpCensusPdf(opts: OpCensusExportOpts): Promise<void> {
  ...
  const colWidths = [42.52, CONTENT_WIDTH - 42.52 - 70.87, 70.87]; // S.No (15mm), Speciality (rem), Count (25mm)

  // Chunk rows
  const chunks: OpCensusRow[][] = [];
  ...
  chunks.forEach((chunk, pageIdx) => {
    ...
    const pageSum = chunk.reduce((sum, r) => sum + r.count, 0);
    ...
    doc.font('Helvetica').fontSize(9).fillColor(GREY).text('Department Wise OP Count', { align: 'center', width: CONTENT_WIDTH });
    ...
    doc.text('Count', MARGIN + colWidths[0] + colWidths[1] + 4, y + 6, { width: colWidths[2] - 8, align: 'center' });
    ...
    chunk.forEach((row, i) => {
      ...
      doc.text(String(row.count), MARGIN + colWidths[0] + colWidths[1] + 4, y + 5, { width: colWidths[2] - 8, align: 'center' });
    ...
    doc.text(String(pageSum), MARGIN + colWidths[0] + colWidths[1] + 4, y + 5, { width: colWidths[2] - 8, align: 'center' });
    y += 18;
    ...
    if (isLast) {
      ...
      doc.text(String(totalCount), MARGIN + colWidths[0] + colWidths[1] + 4, y + 6, { width: colWidths[2] - 8, align: 'center' });
      y += 20;
    }
...
export async function generateOpCensusExcel(opts: OpCensusExportOpts): Promise<void> {
  const { rows, startDate, endDate, outputPath } = opts;

  const workbook = new ExcelJS.Workbook();
  workbook.creator = 'OP Census Report';
  workbook.created = new Date();

  const sheet = workbook.addWorksheet('OP Census Report', {
  ...
  subCell.value = 'Department Wise OP Count';
  ...
  headerRow.height = 22;
  const headers = ['S.No', 'Speciality', 'Count'];
  headers.forEach((h, i) => {
  ...
  // 6. Grand Total
  const totalCount = rows.reduce((sum, r) => sum + r.count, 0);
  const totalRow = sheet.addRow(['', 'Grand Total', totalCount]);
  totalRow.height = 20;
