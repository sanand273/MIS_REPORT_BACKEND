...
import { generateCreditOutstandingCsv, generateCreditOutstandingExcel, generateCreditOutstandingPdf, CreditOutstandingRow } from '../utils/creditOutstandingExporter.js';
...
export class ReportService {
  ...
  /**
   * Generates PDF/Excel/CSV files for Credit Outstanding report.
   */
  async exportReportFile(filters: ReportFilters, format: 'csv' | 'xlsx' | 'pdf'): Promise<{ filePath: string; contentType: string }> {
    ...
    try {
      ...
      const data = await this.getCreditOutstandingReport({
        tenant,
        from: fromDate,
        to: toDate,
        page: 1,
      ...
      const ts = istDate.toISOString().replace(/[-T:]/g, '').substring(0, 14); // YYYYMMDDHHMMSS

      const outDir = path.resolve('./'); // Saves to server root
      const extension = format === 'xlsx' ? 'xlsx' : format;
      const fileName = `credit_outstanding_pharmacy_${tenant}_${fromDate}_to_${toDate}_${ts}.${extension}`;
      const filePath = path.join(outDir, fileName);

      const opts = {
        rows: data.records as CreditOutstandingRow[],
      ...
      if (format === 'csv') {
        await generateCreditOutstandingCsv(opts);
      } else if (format === 'xlsx') {
        await generateCreditOutstandingExcel(opts);
      } else if (format === 'pdf') {
        await generateCreditOutstandingPdf(opts);
      }
      ...
      logger.error(`Export file generation failed: ${err}`);
